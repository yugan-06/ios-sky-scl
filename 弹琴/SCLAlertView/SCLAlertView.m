...
#define ADD_BUTTON_PADDING 17.0f//菜单拉长
#define DEFAULT_WINDOW_WIDTH 230//菜单宽度
...
@property (nonatomic) CGFloat contentHeight;//新加
...
{
    // Default values
 kCircleBackgroundTopPosition = -0.0f;//圆圈顶部位置
    kCircleHeight = 0.0f;//圆圈高度
    kCircleHeightBackground = 0.0f;//圆圈背景
    kActivityIndicatorHeight = 10.0f;//活动指示灯
    kTitleTop = 20.0f;//顶部标题
    self.titleHeight = 30.0f;//标题高度
    self.subTitleY = 70.0f;//字暮
    self.subTitleHeight = 90.0f;//字暮高度
    self.circleIconHeight = 0.0f;//圆圈
    self.windowWidth = windowWidth;
    self.windowHeight = 150.0f;//宽度
    self.contentHeight = 96.0f;//新加/新加/新加/新加/新加/新加/新加/新加/新加/新加/新加
    self.shouldDismissOnTapOutside = NO;//应该拒绝在外面//试过本来NO
    //self.usingNewWindow = YES;//使用新窗口本来NO
    self.canAddObservers = YES;//本来YES
    self.keyboardIsVisible = YES;//选YES键盘可见打字不会被遮挡
    self.hideAnimationType =   SCLAlertViewHideAnimationFadeOut;//隐藏弹出//试过 translation    FadeOut
    self.showAnimationType =  SCLAlertViewHideAnimationFadeOut;//动画幻灯片//试过
    self.backgroundType =  SCLAlertViewBackgroundTransparent;//背景阴影
    self.tintTopCircle = YES;//本来YES
    ...
    //公告板的地方
    //_titleFontSize = 20.0f;//新加//新加//新加//新加
    //_bodyFontSize = 14.0f;//新加//新加//新加//新加
    //_buttonsFontSize = 14.0f;//新加//新加//新加//新加
    ...
    [scrollView setContentSize:CGSizeMake(10, [UIScreen  mainScreen].bounds.size.width+1000000)];//菜单高度 左边啥的
    _contentView = scrollView;
    //滑动菜单
    _buttons = [[NSMutableArray alloc] init];//菜单重叠了？
    _inputs = [[NSMutableArray alloc] init];
    _customViews = [[NSMutableArray alloc] init];
    self.view.accessibilityViewIsModal = YES;
    
    
    _titleFontSize = 22.0f;//标题
    _bodyFontSize = 15.0f;//公告板
    _buttonsFontSize = 14.0f;//标题按钮大小
    ...
    _circleViewBackground.backgroundColor = [UIColor clearColor];
    ...
    // Content View。 内容视图
    _contentView.backgroundColor = [UIColor whiteColor];//[UIColor colorWithRed: 0.5 green: 0.20 blue: 0.10 alpha: 0.5];
    _contentView.layer.cornerRadius = 5.0f;//角半径
    _contentView.layer.masksToBounds = YES;
    _contentView.layer.borderWidth = 1.0f;//边框宽度
    [_contentView addSubview:_viewText];
    [_contentView addSubview:_labelTitle];//D3D3D3
  
    // Colors颜色
    //self.backgroundViewColor = [UIColor whiteColor];//背景视图颜色。 blackColor黑色
    self.backgroundViewColor = [UIColor colorWithWhite:1.f alpha:0.5];//背景视图颜色。 clearColor透明
    //self.backgroundViewColor = [[UIColor colorWithRed:148.f green:170.f blue:214.f alpha:0.5];
    _labelTitle.textColor = UIColorFromHEX(0x000000); //Dark Grey。 大标题文本颜色
    _viewText.textColor = UIColorFromHEX(0x000000); //Dark Grey。小标题文本颜色
    _contentView.layer.borderColor = [UIColor colorWithRed: 1 green: 0.75 blue: 0.79 alpha: 0.5].CGColor;//UIColorFromHEX(80a0ff).CGColor; //Light Grey。图层边框颜色
}
...
- (void)setupNewWindow {
    ...
    alertWindow.backgroundColor = [UIColor clearColor];//透明颜色clearColor
...
- (void)viewWillLayoutSubviews
{
    ...
    //_contentView.contentSize = CGSizeMake(_windowWidth, self.contentHeight);//新加新加新加新加
    
    
    // Adjust corner radius, if a value has been passed
    _contentView.layer.cornerRadius = self.cornerRadius ? self.cornerRadius : 15.0f;//新加5改成15
}
...
- (SCLTextView *)addTextField:(NSString *)title setDefaultText:(NSString *)defaultText
{
    ...
    //新加==================================================================================
    self.contentHeight += txt.bounds.size.height + 10.0f;
    //新加==================================================================================
    self.windowHeight += txt.bounds.size.height + 10.0f;//自己的
    
    //新加==================================================================================
    CGFloat maxH = [UIScreen mainScreen].bounds.size.height - 40.0f;
    
    if (self.windowHeight > maxH) {
        self.windowHeight = maxH;
    }
    //新加==================================================================================
...
- (SCLButton *)addButton:(NSString *)title
{
    ...
    if (_horizontalButtons) {
        //Update buttons width according to the number of buttons//这里新去掉注释
        ...
        // Update view height
        if (!([_buttons count] > 1)) {
            self.contentHeight += (btn.frame.size.height + ADD_BUTTON_PADDING);
        }
    } else {
        //Update view height
        self.contentHeight += (btn.frame.size.height + ADD_BUTTON_PADDING);
    }
    self.windowHeight = self.contentHeight;
    CGFloat maxH = [UIScreen mainScreen].bounds.size.height - 40.0f;
    if (self.windowHeight > maxH) {
        self.windowHeight = maxH;
    }
...
- (SCLAlertViewResponder *)showTitle:(UIViewController *)vc image:(UIImage *)image color:(UIColor *)color title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    switch (style)
    {
        ...
        case SCLAlertViewStyleInfo:
            viewColor = UIColorFromHEX(0x000000);//按钮颜色粉色。 0xFFCCCC   按钮颜色紫色0xAB82FF   按钮颜色淡蓝色0xBCD2EE
            iconImage = SCLAlertViewStyleKit.imageOfNotice;//上方图标
            break;
 /*
  iconImage = SCLAlertViewStyleKit.imageOfCheckmark;勾
  iconImage = SCLAlertViewStyleKit.imageOfNotice;手指头
  iconImage = SCLAlertViewStyleKit.imageOfWarning;!!!
  iconImage = SCLAlertViewStyleKit.imageOfCross; XXX
  iconImage = SCLAlertViewStyleKit.imageOfInfo;iii
  */
...
- (void)makeBlurBackground
{
    ...
    UIImage *blurSnapshotImage = [image applyBlurWithRadius:0.3f
                                                  tintColor:[UIColor colorWithWhite:0.2f
                                                                              alpha:0.3f]
                                      saturationDeltaFactor:0.3f
                                                  maskImage:nil];
